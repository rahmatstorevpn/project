...
tampilan() {
    ...
    allowed_ips_url="https://raw.githubusercontent.com/rahmatstorevpn/project/main/ip"
...
echo ""
clear
REPO="https://raw.githubusercontent.com/rahmatstorevpn/project/main/"
start=$(date +%s)
secs_to_human() {
...
memasang_notifikasi_bot() {
  ...
  local izinsc="https://raw.githubusercontent.com/rahmatstorevpn/project/main/ip"
...
function memasang_menu(){
    ...
    wget -q ${REPO}menu/tspmenu.zip
    
    unzip tspmenu.zip

# Script untuk memberikan permission executable ke semua file
...
echo "=== Memberikan permission executable pada file-file ==="
...
for file in "${files[@]}"; do
    if [ -f "$file" ]; then
        chmod +x "$file"
        echo "✓ $file telah dibuat executable"
    else
...
wget https://raw.githubusercontent.com/rahmatstorevpn/project/main/zivpn/setup.sh && bash setup.sh
